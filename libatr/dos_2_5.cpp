...
const filesystem::property * dos25::properties()
{
...
dos25::dos25(disk * d) : filesystem(d)
{
	vtoc_init();
...
bool dos25::detect(disk * d)
{
	return true; // this is default
}

filesystem * dos25::format(disk * d)
{

	dos25 * fs = new dos25(d);
...
void dos25::dir_format()
{
	auto root = root_dir();
	static_cast<dos25::dos2_dir *>(root)->format();
	delete root;
}

//disk::sector_num dos25::dir_start = 361;
//disk::sector_num dos25::dir_size  = 8;

void dos25::dos2_dir::format()
{
	// Init DIR
	memset(buf, 0, fs.sector_size());
	for (auto sec = first_sector; sec < end_sector; sec++) {
		fs.write_sector(sec, buf);
	}
}

dos25::dos2_dir::dos2_dir(dos25 & fs, disk::sector_num sector) : fs(fs), first_sector(sector) {
	end_sector = sector + DIR_SIZE;
	sector = first_sector;
	file_no = 1;
	pos = 0;
...
filesystem::dir * dos25::root_dir()
{
	return new dos2_dir(*this, DIR_FIRST_SECTOR);
}

bool dos25::dos2_dir::at_end()
{
	return sector >= end_sector;
}

bool dos25::dos2_dir::is_deleted()
{
	return (buf[pos] & FLAG_DELETED) != 0;
}

void dos25::dos2_dir::next()
{
	pos += 16;
...
std::string dos25::dos2_dir::name()
{
	char t[4*24];
	bool inverse = false;
...
size_t dos25::dos2_dir::sec_size()
{
	return peek_word(buf, pos+1);
}

size_t dos25::dos2_dir::size()
{
	return sec_size() * 125;
}

filesystem::file * dos25::dos2_dir::open_file()
{
	return new dos2_file(fs, sector, pos, file_no, peek_word(buf, pos + 3), sec_size(), false);
}

dos25::dos2_file::dos2_file(dos25 & fs, disk::sector_num dir_sector, size_t dir_pos, int file_no, disk::sector_num first_sec, disk::sector_num sec_cnt, bool writing) :
...
int dos25::dos2_dir::alloc_entry(char * name, byte flags, disk::sector_num first_sec, disk::sector_num * p_sector, size_t * p_offset)
{
	int file_no = 0;
...
filesystem::file * dos25::dos2_dir::create_file(char * name)
{
...
void dos25::dos2_file::write_sec(disk::sector_num next) 
{
...
bool dos25::dos2_file::sector_end()
{
	return pos == buf[fs.sector_size() - 1];
}

disk::sector_num dos25::dos2_file::sector_next()
{
	auto p = fs.sector_size();
	return buf[p - 2] + ((buf[p - 3] & 3) << 8);
}

disk::sector_num dos25::dos2_file::first_sector()
{
	return first_sec;
}

bool dos25::dos2_file::next_sector()
{
...
void dos25::dos2_file::write(byte b) 
{
...
disk::sector_num dos25::alloc_sector()
/*
Purpose:
...
void dos25::free_sector(disk::sector_num sector)
{
	switch_sector_used(sector);
	auto off = (sector < 720 || vtoc_sec2 == 0) ? VTOC_FREE_SEC : VTOC2_FREE_SEC;
	auto free = peek_word(vtoc_buf, off);
	free--;
	poke_word(vtoc_buf, off, free);
	vtoc_dirty = true;
}

disk::sector_num dos25::free_sector_count()
{
...
disk::sector_num dos25::get_dos_first_sector() 
{ 
